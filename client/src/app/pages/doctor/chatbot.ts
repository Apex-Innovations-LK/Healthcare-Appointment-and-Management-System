...
export class Chatbot implements OnInit {
  ...
  readonly historyBaseUrl = 'http://localhost:8081/api';
  ...
  loadChatSessions() {
    this.http.get<any[]>(`${this.historyBaseUrl}/chat-history/sessions/${this.userId}`).subscribe({
      next: (data) => {
  ...
  loadSession(sessionId: string) {
    ...
    this.http.get<ApiChatMessage[]>(`${this.historyBaseUrl}/chat-history/session/${sessionId}`).subscribe({
      next: (data) => {
