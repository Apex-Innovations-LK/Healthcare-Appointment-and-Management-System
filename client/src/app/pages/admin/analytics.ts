...
export class Analytics implements OnInit, OnDestroy {
    ...
    private initCharts(data: AnalyticsData): void {
            ...
            datasets: [
                {
                    ...
                    backgroundColor: ['#60a5fa', '#fbbf24', '#34d399', '#f87171', '#a78bfa', '#f472b6', '#fcd34d', '#c084fc', '#4ade80', '#facc15']
                }
            ]
        };
        this.pieOpts = {
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: (ctx: TooltipItem<'pie'>) => `${ctx.label}: ${ctx.parsed}`
                    }
                }
            }
        };
       ...
       -------------------------------------------------- */
        const riskMap = data.riskCategoryCounts ?? {};
        const polarLabels = Object.keys(riskMap);
        const polarVals = Object.values(riskMap);

        this.polarData = {
            labels: polarLabels.length ? polarLabels : ['Low', 'Moderate', 'High'],
            datasets: [
                {
                    data: polarVals.length ? polarVals : [0, 0, 0],
