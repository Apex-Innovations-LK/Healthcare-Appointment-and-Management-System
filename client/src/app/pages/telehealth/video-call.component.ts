...
export class VideoCallComponent implements OnInit, OnDestroy {
    ...
    getStatusClass(): string {
        ...
        if (this.callStatus.includes('Creating') || this.callStatus.includes('Joining') || this.callStatus.includes('Waiting')) return 'connecting';
        return 'ready';
    }
        ...
        this.pc.ontrack = (event) => {
            this.zone.run(() => {
                ...
                if (remoteVideo && event.streams && event.streams[0]) {
                    ...
                    this.connectionStatusRef.set('connected').catch((err: Error) => console.error('Error updating connection status:', err));
        ...
        this.pc.onconnectionstatechange = () => {
            this.zone.run(() => {
                ...
                if (this.pc?.connectionState === 'connected') {
                    ...
                    this.stopCallTimer();
                    this.connectionStatusRef.set('error').catch((err: Error) => console.error('Error updating connection status:', err));
                } else if (this.pc?.connectionState === 'connecting') {
                    this.callStatus = 'Connecting...';
                    this.connectionStatusRef.set('connecting').catch((err: Error) => console.error('Error updating connection status:', err));
                }
    ...
    private setupSignaling() {
        ...
        const answerSub = this.answerRef.valueChanges().subscribe((answer: any) => {
            this.zone.run(() => {
        ...
        const iceSub = this.iceCandidatesRef.valueChanges().subscribe((candidates: any[]) => {
            this.zone.run(() => {
                if (this.pc) {
                    candidates.forEach((candidateData) => {
                        const candidate = new RTCIceCandidate(candidateData);
                        if (this.remoteDescriptionSet) {
                            this.pc!.addIceCandidate(candidate).catch((e: Error) => console.error('Error adding ICE candidate:', e));
                        } else {
        ...
        const connectionStatusSub = this.connectionStatusRef.valueChanges().subscribe((status: any) => {
            this.zone.run(() => {
    ...
    private processPendingCandidates() {
        if (this.pc && this.remoteDescriptionSet) {
            this.pendingCandidates.forEach((candidate) => {
                this.pc!.addIceCandidate(candidate).catch((e: Error) => console.error('Error adding pending ICE candidate:', e));
            });
            this.pendingCandidates = [];
        }
    }
    ...
    private async joinCall() {
        ...
        try {
            ...
            this.connectionStatusRef.set('joining').catch((err: Error) => console.error('Error updating connection status:', err));
            this.cdr.detectChanges();
